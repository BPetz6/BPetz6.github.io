...
# embedding_dim = int(os.environ.get('EMBEDDING'))
embedding_dim = 4 #performs better than 8,16, and 32
nmf_model = NMF(n_components=embedding_dim, init='random', random_state=0)
embeddings = nmf_model.fit_transform(distance_matrix)
...
class DistanceEmbeddingLayer(Layer):
    ...
    def call(self, inputs):
        ...
        repeated_embeddings = tf.tile(tf.expand_dims(embeddings, axis=0), [batch_size, 1, 1])
    ...
    def get_config(self):
        ...
        config.update({
            'embeddings': self.embeddings,  # Save the embeddings
        })
        return config
...
if os.path.exists(os.path.join(models_dir, 'gamma_model.keras')):
    gammaModel = tf.keras.models.load_model(os.path.join(models_dir, 'gamma_model.keras'), custom_objects={'DistanceEmbeddingLayer': DistanceEmbeddingLayer(embeddings)})
else:
    gammaModel = keras.Sequential()
    gammaModel.add(keras.Input(shape=(REGIONS,64)))
    gammaModel.add(DistanceEmbeddingLayer(embeddings))
